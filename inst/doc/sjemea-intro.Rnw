...
It is not a comprehensive guide, but simply gives examples of what can
be done with the package.  

The package contains some example data sets which are used here to
demonstrate various routines.  Section~\ref{sec:input} lists the ways
in which data can be read into the package, and custom-support for
various hardware.



\subsection*{Help pages}
A list of help pages associated with the package can be seen by typing:
<<help>>=
help(package=sjemea)
@
...
Zhengzheng Zhang provided the \texttt{logisi} function.
...
\subsection*{Converting neuroexplorer timestamp files}

Neuroexplorer timestamp files can be converted into plain text using
the routines provided on their web site:
\url{http://www.neuroexplorer.com/code.html}  Example code for C and
matlab are given.  Future work could include converting this code to R
if there is demand.
...
each well can be analysed independently in an efficient manner.)  This introduces an
extra layer of abstraction into the geometry of the array, in that an
electrode is part of a well, and there are many wells on one array (or
``plate'').  
...
parallel() code.  I think has now been fixed, so we might be able to
