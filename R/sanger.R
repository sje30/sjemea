...
##' Read in a nexTimestamps file from Neuroexplorer and create a "spikes" data
##' structure.
##' 
...
##' 
##' This format is fairly simple; each column represents one channel, with the
##' spikes recorded in that channel filling up the column.  Since different
##' channels are likely to record different numbers of spikes, empty values
##' (just a TAB to next value) are found at the bottom of each spike train.
##' 
##' Text files can be compressed, using gzip, to save space.  (The extension
##' .gz is added to compresed files.)  This compression aspect should be
##' transparent to the end user -- if a file is compresed, the filename can be
##' given with or without the .gz extension.  gzip is available for Windows
##' (from \url{http://www.gzip.org}); "gzip *.nexTimestamps" will compress all
...
##' @param min.rate Minimum firing rate of an electrode.
##' @return Return the data structure 's'.
##' @author Stephen Eglen
##' @seealso \code{\link{jay.read.spikes}} \code{\link{compute.ns}}
##' @examples
##' 
