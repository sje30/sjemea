...
\section*{Firing regularity}
Firing regularity is another measure to charactarize individual spike trains.
The following examples use Interspike Intervals to compute the firing
regularity, a perfectly regular firing would have equal interspike intervals.

\subsection*{Fitting Gamma distribution to Interspike Intervals (ISI)}
By fitting Gamma Distribution to ISI we can distinguish between firing in
bursts, sporadic but regular firing, and randomly distributed spikes following
Poisson distribution.
The estimated logshape = 0 of Gamma distribution describes Poisson distributed
spikes and the greater the absolute value of logshape, the more regular spiking.
For bursting logshape < 0 , for sporadic firing logshape > 0.

In the plot below we see that the spike trains in the example come from a 
bursting  network.

<<gamma-dist, warning=FALSE>>=
logshapes <- c()
logrates <- c()
for (i in 1:s$NCells) {
  gamma.estimate <- isi.gamma(s$spikes[[i]], chunk.length = 1000)
...
\subsection*{Other measures of regularity}

For each electrode we compute local variations of firing regularity. It is a
measure of correlation between consecutive interspike intervals.
<<local-variation>>=
local.variations <- sapply(s$spikes, isi.local.variation)
summary(unname(local.variations))
@

We can also compute Spearman's correlation coefficient on ranks.
<<spearman-rank>>=
spearman.ranks <- sapply(s$spikes, isi.spearman.rank.corr, chunk.length=100)
summary(unname(spearman.ranks))
@
...
<<logisi>>=
isi <- unlist(unname(sapply(s$spikes, diff)))
hist(log(isi))
@
...
Alternatively, to routinely use Tiling coefficient (e.g. in
fourplots), do the following:

<<tiling-correlation-fourplot>>=
